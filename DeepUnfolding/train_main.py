...
def back_propagation():
    ...
    ############################ Initialize G J L M
    G_UH = [
        [np.matrix(np.zeros((Nr, d), dtype=np.complex)) for i in range(User)]
        for j in range(Layer)
    ]
    G_WH = [
        [np.matrix(np.zeros((d, d), dtype=np.complex)) for i in range(User)]
        for j in range(Layer)
    ]
    G_VH = [
        [np.matrix(np.zeros((Nt, d), dtype=np.complex)) for i in range(User)]
        for j in range(Layer)
    ]
    G_VH_temp = [
        [np.matrix(np.zeros((Nt, d), dtype=np.complex)) for i in range(User)]
    ...
    G_U = [
        [np.matrix(np.zeros((d, Nr), dtype=np.complex)) for i in range(User)]
        for j in range(Layer)
    ]
    G_W = [
        [np.matrix(np.zeros((d, d), dtype=np.complex)) for i in range(User)]
    ...
    G_V = [
        [np.matrix(np.zeros((d, Nt), dtype=np.complex)) for i in range(User)]
        for j in range(Layer)
    ]
    G_V_temp = [
        [np.matrix(np.zeros((d, Nt), dtype=np.complex)) for i in range(User)]
    ...
    J_UH = [
        [np.matrix(np.zeros((Nr, Nr), dtype=np.complex)) for i in range(User)]
        for j in range(Layer)
    ]
    J_WH = [
        [np.matrix(np.zeros((d, d), dtype=np.complex)) for i in range(User)]
    ...
    J_VH = [
        [np.matrix(np.zeros((Nt, Nt), dtype=np.complex)) for i in range(User)]
        for j in range(Layer)
    ]
    J_U = [
        [np.matrix(np.zeros((Nr, Nr), dtype=np.complex)) for i in range(User)]
    ...
    M_VH = [
        [np.matrix(np.zeros((Nr, Nt), dtype=np.complex)) for i in range(User)]
        for j in range(Layer)
    ]
    M_U = [
        [np.matrix(np.zeros((d, Nt), dtype=np.complex)) for i in range(User)]
    ...
    M_V = [
        [np.matrix(np.zeros((Nt, Nr), dtype=np.complex)) for i in range(User)]
        for j in range(Layer)
    ]

    ############################## The Gradient of the last layer
    G_UH[Layer - 1] = UW_gradi.U_gradient_lastlayer(
        Nr, Nt, d, Pt, sigma, User, H, W[Layer - 1], U[Layer - 1]
    )
    G_WH[Layer - 1] = UW_gradi.W_gradient_lastlayer(
        Nr, Nt, d, Pt, sigma, User, H, W[Layer - 1], U[Layer - 1]
    )
    ...
    for n in range(User):
        J_W[Layer - 1][n] = np.multiply(
            (X_W[Layer - 1][n] * G_W[Layer - 1][n]),
            np.multiply(-I_E[Layer - 1][n], I_E[Layer - 1][n]).T,
        )
        J_WH[Layer - 1][n] = np.multiply(
            (G_WH[Layer - 1][n] * X_W[Layer - 1][n].H),
            np.multiply(-I_E[Layer - 1][n].H, I_E[Layer - 1][n].H).T,
        )

    for n in range(User):
        G_U[Layer - 1][n] = (
            G_U[Layer - 1][n]
            - J_WH[Layer - 1][n] * V1[Layer - 2][n].H * H[n].H
            - G_WH[Layer - 1][n] * Y_W[Layer - 1][n].H * V1[Layer - 2][n].H * H[n].H
        )
        G_UH[Layer - 1][n] = (
            G_UH[Layer - 1][n]
            - H[n] * V1[Layer - 2][n] * J_W[Layer - 1][n]
            - H[n] * V1[Layer - 2][n] * Y_W[Layer - 1][n] * G_W[Layer - 1][n]
        )

    ####### Update J_U  L_U  M_U  J_W  in the last layer
    for n in range(User):
        J_U[Layer - 1][n] = np.multiply(
            (X_U[Layer - 1][n] * H[n] * V1[Layer - 2][n] * G_U[Layer - 1][n]),
            np.multiply(-I_A[Layer - 2][n], I_A[Layer - 2][n]).T,
        )
        J_UH[Layer - 1][n] = np.multiply(
            (G_UH[Layer - 1][n] * V1[Layer - 2][n].H * H[n].H * X_U[Layer - 1][n].H),
            np.multiply(-I_A[Layer - 2][n].H, I_A[Layer - 2][n].H).T,
        )

        L_U[Layer - 1][n] = (
            Y_U[Layer - 1][n] * H[n] * V1[Layer - 2][n] * G_U[Layer - 1][n]
        )
        L_UH[Layer - 1][n] = (
            G_UH[Layer - 1][n] * V1[Layer - 2][n].H * H[n].H * Y_U[Layer - 1][n].H
        )

        M_U[Layer - 1][n] = (
            G_U[Layer - 1][n]
            * (
                (I_A[Layer - 2][n]) * X_U[Layer - 1][n]
                + A[Layer - 2][n] * Y_U[Layer - 1][n]
                + Z_U[Layer - 1][n]
            )
            * H[n]
        )
    ...
    for l in Layer_inverse:
        ...
        for n in range(User):
            for k in range(User):
                G_V_temp[l][n] = (
                    G_V_temp[l][n]
                    + np.trace(
                        J_U[l + 1][k] + L_U[l + 1][k] + J_UH[l + 1][k] + L_UH[l + 1][k]
                    )
                    * sigma
                    * sigma
                    / Pt
            ...
            G_V_temp[l][n] = (
                G_V_temp[l][n]
                - J_W[l + 1][n] * U[l + 1][n].H * H[n]
                - Y_W[l + 1][n] * G_W[l + 1][n] * U[l + 1][n].H * H[n]
                + M_U[l + 1][n]
            )
            G_VH_temp[l][n] = (
                G_VH_temp[l][n]
                - H[n].H * U[l + 1][n] * J_WH[l + 1][n]
                - H[n].H * U[l + 1][n] * G_WH[l + 1][n] * Y_W[l + 1][n].H
                + M_UH[l + 1][n]
            )
        ...
        for n in range(User):
            J_V[l][n] = np.multiply(
                (X_V[l][n] * H[n].H * U[l][n] * W[l][n] * G_V[l][n]),
                np.multiply(-I_B[l], I_B[l]).T,
            )
            L_V[l][n] = Y_V[l][n] * H[n].H * U[l][n] * W[l][n] * G_V[l][n]
            M_V[l][n] = ((I_B[l]) * X_V[l][n] + B[l] * Y_V[l][n] + Z_V[l][n]) * H[n].H

            J_VH[l][n] = np.multiply(
                (G_VH[l][n] * W[l][n].H * U[l][n].H * H[n] * X_V[l][n].H),
                np.multiply(-I_B[l].H, I_B[l].H).T,
            )
        ...
        for n in range(User):
            J_W[l][n] = np.multiply(
                X_W[l][n] * G_W[l][n], np.multiply(-I_E[l][n], I_E[l][n]).T
            )
            J_WH[l][n] = np.multiply(
                G_WH[l][n] * X_W[l][n].H, np.multiply(-I_E[l][n].H, I_E[l][n].H).T
            )
        ...
        for n in range(User):
            for k in range(User):
                G_U[l][n] = (
                    ...
                    * W[l][n].H
                    * U[l][n].H
                    + W[l][n] * U[l][n].H * H[n] * (J_V[l][k] + L_V[l][k]) * H[n].H
                    + W[l][n].H * U[l][n].H * H[n] * (J_VH[l][k] + L_VH[l][k]) * H[n].H
                )
                ...
                G_UH[l][n] = (
                    ...
                    * U[l][n]
                    * W[l][n].H
                    + H[n] * (J_V[l][k] + L_V[l][k]) * H[n].H * U[l][n] * W[l][n]
                    + H[n] * (J_VH[l][k] + L_VH[l][k]) * H[n].H * U[l][n] * W[l][n].H
                )

            G_U[l][n] = (
                G_U[l][n]
                - J_WH[l][n] * V1[l - 1][n].H * H[n].H
                - G_WH[l][n] * Y_W[l][n].H * V1[l - 1][n].H * H[n].H
                + W[l][n] * G_V[l][n] * M_V[l][n]
            )
            G_UH[l][n] = (
                G_UH[l][n]
                - H[n] * V1[l - 1][n] * J_W[l][n]
                - H[n] * V1[l - 1][n] * Y_W[l][n] * G_W[l][n]
                + M_VH[l][n] * G_VH[l][n] * W[l][n].H
            )

        ####### Update J_U  L_U  M_U
        for n in range(User):
            J_U[l][n] = np.multiply(
                X_U[l][n] * H[n] * V1[l - 1][n] * G_U[l][n],
                np.multiply(-I_A[l - 1][n], I_A[l - 1][n]).T,
            )
            L_U[l][n] = Y_U[l][n] * H[n] * V1[l - 1][n] * G_U[l][n]
            M_U[l][n] = (
                G_U[l][n]
                * ((I_A[l - 1][n]) * X_U[l][n] + A[l - 1][n] * Y_U[l][n] + Z_U[l][n])
                * H[n]
            )

            J_UH[l][n] = np.multiply(
                G_UH[l][n] * V1[l - 1][n].H * H[n].H * X_U[l][n].H,
                np.multiply(-I_A[l - 1][n].H, I_A[l - 1][n].H).T,
            )
            L_UH[l][n] = G_UH[l][n] * V1[l - 1][n].H * H[n].H * Y_U[l][n].H
            M_UH[l][n] = (
                H[n].H
                * ((I_A[l - 1][n]) * X_U[l][n] + A[l - 1][n] * Y_U[l][n] + Z_U[l][n]).H
                * G_UH[l][n]
            )
    ...
    for l in Layer_inverse_part:
        for k in range(User):
            G_VH_X[l + 1][k] = (
                I_B[l + 1].H * G_VH[l + 1][k] * W[l + 1][k].H * U[l + 1][k].H * H[k]
            )
    ...
    for k in range(User):
        ...
        G_UH_X[Layer - 1][k] = (
            I_A[Layer - 2][k].H * G_UH[Layer - 1][k] * V[Layer - 2][k].H * H[k].H
        )
...
X_U = [
    [
        ...
        * np.matrix(
            -1
            - 1j
            + 2 * np.random.random((Nr, Nr))
            + 2 * np.random.random((Nr, Nr)) * 1j,
            dtype=np.complex,
        )
        for i in range(User)
    ]
...
X_W = [
    [
        scale_factor_X
        * np.matrix(
            -1 - 1j + 2 * np.random.random((d, d)) + 2 * np.random.random((d, d)) * 1j,
            dtype=np.complex,
        )
        for i in range(User)
    ]
...
X_V = [
    [
        ...
        * np.matrix(
            -1
            - 1j
            + 2 * np.random.random((Nt, Nt))
            + 2 * np.random.random((Nt, Nt)) * 1j,
            dtype=np.complex,
        )
        for i in range(User)
    ]
...
O_U = [
    [
        ...
        * np.matrix(
            -1
            - 1j
            + 2 * np.random.random((Nr, d))
            + 2 * np.random.random((Nr, d)) * 1j,
            dtype=np.complex,
        )
        for i in range(User)
    ]
...
O_V = [
    [
        ...
        * np.matrix(
            -1
            - 1j
            + 2 * np.random.random((Nt, d))
            + 2 * np.random.random((Nt, d)) * 1j,
            dtype=np.complex,
        )
        for i in range(User)
    ]
...
for i in range(10 ** (4)):
    ...
    for k in range(User):
        H.append(
            np.matrix(
                np.random.randn(Nr, Nt) + np.random.randn(Nr, Nt) * 1j, dtype=np.complex
            )
        )
    ...
    for l in range(Layer):
        ...
        for k in range(User):
            U[l].append(
                np.matrix(np.zeros((Nr, d)) + np.zeros((Nr, d)) * 1j, dtype=np.complex)
            )
    ...
    HH = np.matrix(
        np.zeros((User * Nr, Nt)) + np.zeros((User * Nr, Nt)) * 1j, dtype=np.complex
    )
    VV = np.matrix(
        np.zeros((Nt, User * d)) + np.zeros((Nt, User * d)) * 1j, dtype=np.complex
    )
    for k in range(User):
        HH[k * Nr : (k + 1) * Nr, :] = H[k]
    ...
    if (i + 1) % batch == 0:
        ...
        for ii in range(100):
            ...
            for k in range(User):
                H_sample.append(
                    np.matrix(
                        np.random.randn(Nr, Nt) + np.random.randn(Nr, Nt) * 1j,
                        dtype=np.complex,
                    )
                )
            ...
            for l in range(Layer):
                ...
                for k in range(User):
                    V_sample[l].append(
                        np.matrix(
                            np.zeros((Nt, d)) + np.zeros((Nt, d)) * 1j, dtype=np.complex
                        )
                    )

            HH_sample = np.matrix(
                np.zeros((User * Nr, Nt)) + np.zeros((User * Nr, Nt)) * 1j,
                dtype=np.complex,
            )
            VV_sample = np.matrix(
                np.zeros((Nt, User * d)) + np.zeros((Nt, User * d)) * 1j,
                dtype=np.complex,
            )
            for k in range(User):
        ...
        for l in Layer_inverse1:
            for k in range(User):
                X_V[l + 1][k] = X_V[l + 1][k] + step_size_X * G_VH_X_batch[l + 1][
                    k
                ] / np.linalg.norm(G_VH_X_batch[l + 1][k], ord=2)
                #                Y_V[l+1][k] = Y_V[l+1][k] + step_size_Y*G_VH_Y_batch[l+1][k]/np.linalg.norm( G_VH_Y_batch[l+1][k],ord=2 )
                Z_V[l + 1][k] = Z_V[l + 1][k] + step_size_Z * G_VH_Z_batch[l + 1][
                    k
                ] / np.linalg.norm(G_VH_Z_batch[l + 1][k], ord=2)
                O_V[l + 1][k] = O_V[l + 1][k] + step_size_O * G_VH_O_batch[l + 1][
                    k
                ] / np.linalg.norm(G_VH_O_batch[l + 1][k], ord=2)

                X_W[l + 1][k] = X_W[l + 1][k] + step_size_X * G_WH_X_batch[l + 1][
                    k
                ] / np.linalg.norm(G_WH_X_batch[l + 1][k], ord=2)
                #                Y_W[l+1][k] = Y_W[l+1][k] + step_size_Y*G_WH_Y_batch[l+1][k]/np.linalg.norm( G_WH_Y_batch[l+1][k],ord=2 )
                Z_W[l + 1][k] = Z_W[l + 1][k] + step_size_Z * G_WH_Z_batch[l + 1][
                    k
                ] / np.linalg.norm(G_WH_Z_batch[l + 1][k], ord=2)

                X_U[l + 1][k] = X_U[l + 1][k] + step_size_X * G_UH_X_batch[l + 1][
                    k
                ] / np.linalg.norm(G_UH_X_batch[l + 1][k], ord=2)
                #                Y_U[l+1][k] = Y_U[l+1][k] + step_size_Y*G_UH_Y_batch[l+1][k]/np.linalg.norm( G_UH_Y_batch[l+1][k],ord=2 )
                Z_U[l + 1][k] = Z_U[l + 1][k] + step_size_Z * G_UH_Z_batch[l + 1][
                    k
                ] / np.linalg.norm(G_UH_Z_batch[l + 1][k], ord=2)
                O_U[l + 1][k] = O_U[l + 1][k] + step_size_O * G_UH_O_batch[l + 1][
                    k
                ] / np.linalg.norm(G_UH_O_batch[l + 1][k], ord=2)

        ##################  Layer-1
        for k in range(User):
            X_W[Layer - 1][k] = X_W[Layer - 1][k] + step_size_X * G_WH_X_batch[
                Layer - 1
            ][k] / np.linalg.norm(G_WH_X_batch[Layer - 1][k], ord=2)
            #            Y_W[Layer-1][k] = Y_W[Layer-1][k] + step_size_Y*G_WH_Y_batch[Layer-1][k]/np.linalg.norm( G_WH_Y_batch[Layer-1][k],ord=2 )
            Z_W[Layer - 1][k] = Z_W[Layer - 1][k] + step_size_Z * G_WH_Z_batch[
                Layer - 1
            ][k] / np.linalg.norm(G_WH_Z_batch[Layer - 1][k], ord=2)

            X_U[Layer - 1][k] = X_U[Layer - 1][k] + step_size_X * G_UH_X_batch[
                Layer - 1
            ][k] / np.linalg.norm(G_UH_X_batch[Layer - 1][k], ord=2)
            #            Y_U[Layer-1][k] = Y_U[Layer-1][k] + step_size_Y*G_UH_Y_batch[Layer-1][k]/np.linalg.norm( G_UH_Y_batch[Layer-1][k],ord=2 )
            Z_U[Layer - 1][k] = Z_U[Layer - 1][k] + step_size_Z * G_UH_Z_batch[
                Layer - 1
            ][k] / np.linalg.norm(G_UH_Z_batch[Layer - 1][k], ord=2)
            O_U[Layer - 1][k] = O_U[Layer - 1][k] + step_size_O * G_UH_O_batch[
                Layer - 1
            ][k] / np.linalg.norm(G_UH_O_batch[Layer - 1][k], ord=2)
